...
class sql_db
{
	...
	function sql_string($string, $isNumber=false)
	{
				$link=$this->db_connect_id;
		    ...
		    if($isNumber)
		    {
		     ...
		     return "'".mysql_real_escape_string(htmlentities(trim(strip_tags($string))), $link)."'"; 
		}	
		
		function sql_dequote($quotedsql)
		{
			$slen = strlen($quotedsql);
			if($quotedsql{0} == "'")
				if($quotedsql{$slen-1} == "'")
					return substr($quotedsql, 1, $slen-2);
